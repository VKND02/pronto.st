...
def butter_lowpass_filter(data, cutoff=30, fs=100, order=4):
    """Applique un filtre passe-bas Butterworth pour lisser le signal."""
    nyquist = 0.5 * fs
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        if len(ecg_signal) < sampling_rate * 2:
            ...
            ecg_filtered = butter_lowpass_filter(
                ecg_signal, cutoff=30, fs=sampling_rate)

            # Détection des ondes PQRST avec BioSPPy
            out = biosppy_ecg.ecg(signal=ecg_filtered,
            ...
            # === Tracé du signal brut et filtré ===
            fig, axes = plt.subplots(2, 1, figsize=(12, 10))

            # Tracé du signal brut
            axes[0].plot(time, ecg_signal, label="ECG Brut", color='gray')
            axes[0].set_xlabel("Temps (s)")
            axes[0].set_ylabel("Amplitude")
            axes[0].set_title("Signal ECG Brut")
            axes[0].legend()

            # Tracé du signal filtré avec pics détectés
            axes[1].plot(time, filtered_signal,
                         label="ECG Filtré (BioSPPy)", color='black')
            axes[1].scatter(time[r_peaks], filtered_signal[r_peaks],
                            color='red', label="R-peaks")
            axes[1].scatter(time[q_peaks], filtered_signal[q_peaks],
                            color='green', label="Q-peaks")
            axes[1].scatter(time[s_peaks], filtered_signal[s_peaks],
                            color='magenta', label="S-peaks")
            axes[1].scatter(time[p_peaks], filtered_signal[p_peaks],
                            color='cyan', label="P-peaks")
            axes[1].scatter(time[t_peaks], filtered_signal[t_peaks],
